...
fn blake3_vectors() {
    ...
    println!("  TV-BLAKE3-004 (tx_ordering_key, epoch=1, tx=ab*32): {}", hex::encode(h3.as_bytes()));
    ...
    println!("  TV-BLAKE3-005 (seed_k, manifest=42*32): {}", hex::encode(h4.as_bytes()));
    ...
    println!("  TV-BLAKE3-006 (nmt_rank, node=01*32, seed=99*32): {}", hex::encode(h5.as_bytes()));
}
...
fn poseidon2_vectors() {
    ...
    println!("  TV-POSEIDON2-004 nullifier(s=02*8, sk=03*8): 0x{:016x}", nullifier);
}
