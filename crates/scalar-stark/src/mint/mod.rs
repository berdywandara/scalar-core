...
pub use prover::{
    ...
    compute_mint_nullifier,
    validate_mint_inputs,
    generate_mint_proof,
};
...
mod tests {
    ...
    fn make_pub(epoch_id: u64, node_id: &[u8; 32], reward_root: u64,
                total_minted: u64, output_count: u64) -> MintClaimPublicInputs {
    ...
    fn make_priv(node_id: [u8; 32], reward_amount: u64,
                 output_values: Vec<u64>) -> MintClaimPrivateInputs {
        MintClaimPrivateInputs {
            node_id,
            node_key:            [0u8; 32],
            reward_amount,
    ...
    fn test_nullifier_unique_per_node() {
        assert_ne!(compute_mint_nullifier(&node(1), 0), compute_mint_nullifier(&node(2), 0));
    }
    ...
    fn test_prove_and_verify_roundtrip() {
        ...
        let proof  = generate_mint_proof(&prv_i, pub_i.clone(), 0)
            .expect("Proof generation harus berhasil");
    ...
    fn test_verify_rejects_tampered_public_input() {
        ...
        let proof = generate_mint_proof(&prv_i, pub_i, 0)
            .expect("Proof generation harus berhasil");
        ...
        assert!(result.is_err(), "Verifikasi harus gagal dengan public input yang diubah");
    }
}
