...
pub fn verify_mint_proof(
    ...
    let acceptable_options = winterfell::AcceptableOptions::OptionSet(
        vec![proof.options().clone()]
    );

    winterfell::verify::<MintClaimAir, Blake3_256<BaseElement>, DefaultRandomCoin<Blake3_256<BaseElement>>>(
