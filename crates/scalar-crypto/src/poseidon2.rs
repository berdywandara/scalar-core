...
impl Poseidon2State {
    ...
    pub fn permute(&mut self) {
        ...
        for _ in 0..(ROUNDS_FULL_RF / 2) {
            for i in 0..WIDTH_T {
                self.state[i] = self.state[i].add_mod(GoldilocksElement::new(ROUND_CONSTANTS[rc_counter]));
                rc_counter += 1;
            }
            for i in 0..WIDTH_T {
    ...
    pub fn apply_matrix(&mut self, matrix: &[[u64; WIDTH_T]; WIDTH_T]) {
        ...
        for i in 0..WIDTH_T {
            ...
            for j in 0..WIDTH_T {
                ...
                sum = sum.add_mod(scalar.mul_mod(old_state[j]));
            }
            self.state[i] = sum;
        }
    }
...
mod reference_vectors {
    ...
    fn test_poseidon2_mds_matrix_consistency() {
        ...
        assert_ne!(
            state_before, poseidon.state,
            "Matriks MDS gagal mentransformasi state (Singular/Identitas)!"
        );
    }
}
