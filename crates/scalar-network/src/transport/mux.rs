...
pub enum Priority { Normal, Emergency }

#[derive(Debug, PartialEq)]
pub enum TransportType { Internet, Tor, LoRa, HfRadio, Local }

pub struct TransportMux;

impl TransportMux {
    /// Decision Tree: Otomatis memilih transport berdasarkan prioritas & kondisi sensor
    pub fn select_and_send(_payload: &[u8], priority: Priority, internet_ok: bool, censored: bool) -> Vec<TransportType> {
        match priority {
            Priority::Emergency => {
                // Semua transport paralel (Maximum redundancy)
                vec![TransportType::Internet, TransportType::Tor, TransportType::LoRa, TransportType::HfRadio]
            },
            Priority::Normal => {
                if internet_ok && !censored {
                    ...
                    vec![TransportType::LoRa, TransportType::HfRadio, TransportType::Local]
                }
            }
        }
    }
