...
impl OnionRouter {
    ...
    fn pad_payload(payload: &[u8]) -> Vec<u8> {
        let len = payload.len();
        let target = if len <= PaddingSize::Small as usize { PaddingSize::Small as usize }
        else if len <= PaddingSize::Medium as usize { PaddingSize::Medium as usize }
        else if len <= PaddingSize::Large as usize { PaddingSize::Large as usize }
        else { PaddingSize::XLarge as usize };
