...
impl MintNullifierSet {
    ...
    pub fn is_claimed(&self, node_id: &[u8; 32], epoch_id: u64) -> bool {
        self.spent.contains(&Self::compute_nullifier(node_id, epoch_id))
    }

    /// Rekam klaim. Panggil hanya setelah MINT_CLAIM_CIRCUIT terverifikasi.
    pub fn record_claim(&mut self, node_id: &[u8; 32], epoch_id: u64)
...
mod tests {
    ...
    fn node(b: u8) -> [u8; 32] { let mut id = [0u8; 32]; id[0] = b; id }

    #[test]
    fn test_first_claim_ok()           { assert!(MintNullifierSet::new().record_claim(&node(1), 0).is_ok()); }

    #[test]
    fn test_double_claim_rejected() {
        let mut mns = MintNullifierSet::new();
        mns.record_claim(&node(1), 0).unwrap();
        assert!(matches!(mns.record_claim(&node(1), 0),
