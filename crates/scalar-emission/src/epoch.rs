...
pub fn compute_epoch_rewards(input: &EpochInput) -> Result<EpochOutput, EmissionError> {
    ...
    let weights: Vec<(usize, u64)> = input
        ...
        .enumerate()
        .map(|(i, node_id)| {
            let w = input.liveness_smt.compute_uptime_weight_fp(node_id, input.epoch_id);
            (i, w)
        })
        .collect();
    ...
    for (i, w_i) in &weights {
        ...
        // PoU emission reward
        let pou_reward = EmissionAccumulator::reward_for_node(e_k, *w_i, total_weight_fp)
            .unwrap_or(0);
...
mod tests {
    ...
    fn test_reward_root_deterministic_same_input() {
        ...
        assert_eq!(
            out1.manifest.reward_root,
            out2.manifest.reward_root,
    ...
    fn test_node_rewards_sorted_ascending() {
        ...
        let n3 = node(3); let n2 = node(2); let n1 = node(1);
    ...
    fn test_emission_decreases_over_epochs() {
        ...
        let input0 = EpochInput {
            epoch_id: 0, accepted_liveness_root: [1u8; 32],
            active_node_ids: &[n1], liveness_smt: &smt,
            emission_acc: &acc, fee_acc: &fee_acc,
        };
        ...
        // Epoch 1: M_E > 0 → E(1) < E₀
        let input1 = EpochInput {
            epoch_id: 1, accepted_liveness_root: [2u8; 32],
            active_node_ids: &[n1], liveness_smt: &smt,
            emission_acc: &acc, fee_acc: &fee_acc,
        };
        let out1 = compute_epoch_rewards(&input1).unwrap();
