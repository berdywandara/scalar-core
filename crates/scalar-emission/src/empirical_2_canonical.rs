...
mod empirical_2_canonical_fuzz {
    ...
    fn empirical_2_canonical_serialization_1m_variations() {
        ...
        for seed in 0..N_ITERATIONS {
            ...
            assert_eq!(b1, b_slashed,
                "P4 FAILED seed={}: slashed_nodes masuk canonical", seed);
            ...
            assert_eq!(b1, b_open,
                "P5 FAILED seed={}: status masuk canonical", seed);
            ...
            assert_eq!(b1, b_hash,
                "P6 FAILED seed={}: manifest_hash field masuk canonical", seed);

            // P7: epoch_id little-endian di bytes[0..8] (S3)
            let epoch_le = manifest.epoch_id.to_le_bytes();
            assert_eq!(&b1[0..8], &epoch_le,
                "P7 FAILED seed={}: epoch_id bukan little-endian", seed);

            if violations > 0 { break; }
        }

        assert_eq!(violations, 0,
            "EMPIRICAL-2 FAILED: {} violations dari {} iterasi", violations, N_ITERATIONS);
