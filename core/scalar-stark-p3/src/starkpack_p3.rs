...
mod tests {
    ...
    fn build_proof_input(seed: u64, fee_extra: u64) -> RealProofInput {
        ...
        // Derive sums before ow is moved into witnesses.
        // Conservation: sum_inputs == sum_outputs + fee_total  (MAD CE constraint)
        let fee = 40 + fee_extra;
        let sum_inputs: u64 = ow.iter().map(|w| w.value).sum();
        let sum_outputs: u64 = sum_inputs - fee;
        ...
        // derive_public_claims computes commitment_hash + nullifier_hash
        // from witnesses — verifier must use same claims.pi as prover.
        let verified_pi = claims.pi.clone();
    ...
    fn test_global_fri_root_encodes_count() {
        ...
        let r1 = aggregate_real_proofs(std::slice::from_ref(&inp1)).unwrap();
...
mod bench {
    ...
    fn bench_starkpack_aggregation() {
        ...
        // Warm-up
        let _ = aggregate_real_proofs(std::slice::from_ref(&inp1)).expect("warm-up");

        // N=1 timed
        let start = Instant::now();
        let r1 = aggregate_real_proofs(std::slice::from_ref(&inp1)).expect("N=1 aggregate");
