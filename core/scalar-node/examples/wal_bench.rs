...
fn make_snapshot(epoch_id: u64) -> CheckpointSnapshot {
    CheckpointSnapshot {
        ...
        total_supply_sscl:    1_890_000_000_000_000u64
            .saturating_sub(epoch_id * 1_000_000),
    }
}
...
fn main() {
    ...
    for i in 0..CHECKPOINT_RUNS {
        ...
        let t = Instant::now();
        wal.prepare(i as u64, 1, snap, i as u64 * 1000).expect("prepare failed");
    ...
    for i in 0..1000usize {
        ...
        match r {
            ...
            _ => { idempotent_ok = false; break; }
        }
    }

    // ── is_committed lookup ───────────────────────────────────────────────────
    println!("Benchmarking is_committed lookup ({} ops)...", CHECKPOINT_RUNS);
    ...
    println!("committed_count    : {}", wal.count_by_phase(&scalar_node::wal::WalPhase::Committed));
    println!();

    let epoch_duration_s = 30u64 * 24 * 3600; // 30 days
    let prep_per_epoch   = epoch_duration_s * 1_000_000_000 / prep_med.max(1);
    println!("=== IMPACT ===");
    println!("WAL PREPARE: {}ns median — {} ops/epoch capacity (30-day epoch).",
             prep_med, prep_per_epoch);
    println!("WAL COMMIT:  {}ns median — overhead per checkpoint commit.", comm_med);
    println!("Lookup:      {}ns median — is_committed guard is negligible.", look_med);
    println!("Idempotency: {} — WAL correctly handles re-commit.", idempotent_ok);
    println!();
