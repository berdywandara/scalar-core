            ...
            handles.push(thread::spawn(move || {
                ...
                *c += 1;
                if *c == QUORUM_THRESHOLD {
                    ...
                    if q.is_none() { *q = Some(start.elapsed()); }
                }
            }));
        }
...
fn main() {
    ...
    println!("Config  : {}/{} validators | SLH-DSA verify={}ms (placeholder)", QUORUM_THRESHOLD, VALIDATORS, SLHDSA_VERIFY_MS);
    println!();
    let conditions = [("LOCAL", 1u64, 50usize), ("WAN_50", 50, 50), ("WAN_200", 200, 50)];
    ...
    println!("{:<10} {:>10} {:>10} {:>8}", "condition", "latency_ms", "median_ms", "p95_ms");
    println!("{}", "-".repeat(42));
    ...
    if let Some((_, _, med, _)) = results.iter().find(|(n,_,_,_)| *n == "WAN_50") {
        ...
        println!("quorum_under_30s      : {}", if *med < 30_000 { "YES ✓" } else { "NO ✗" });
