...
fn main() {
    ...
    for leaf in &leaves { imt.append(leaf).expect("IMT append failed"); }
    let root = imt.root();
    ...
    for _ in 0..QUERY_RUNS {
        ...
        if path_size_bytes == 0 { path_size_bytes = path.siblings.len() * 32; }
        let valid = imt_membership_verify(&leaves[idx as usize], &path, &root, count_u64);
        if !valid { eprintln!("  VERIFY FAILED idx={}", idx); all_valid = false; }
    }
    ...
    println!("path_size_bytes      : {} (depth={})", path_size_bytes, path_size_bytes / 32);
    println!("verify_all_correct   : {}", all_valid);
    println!();
    println!("=== IMPACT ===");
    println!("D-023: path_gen {:.3}ms/tx — witness overhead per MicroCommitment batch.", med);
    println!("Status: {}", if med < 1.0 { "VALIDATES — not a bottleneck (<1ms)" }
                           else { "NOTE — >1ms, factor into batch sizing" });
}
