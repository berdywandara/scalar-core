...
fn main() {
    ...
    for _ in 0..WARMUP {
        let _ = sign_message(&payload, sk_bytes).unwrap();
    }
    println!("done.");
    ...
    if verify_med < 10.0 {
        ...
        println!(
            "D-024: {:.1}ms verify EXCEEDS 10ms — factor into heartbeat budget.",
            verify_med
        );
    }
    if sig_size != EXPECTED_SIG_BYTES {
        ...
        for &ivl in &[600u64, 120, 60, 30, 10] {
            ...
            println!(
                "  interval={:4}s  bw={:8.0} B/s  tier_c={}",
                ivl,
                bw,
