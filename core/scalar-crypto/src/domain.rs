...
pub const DOMAIN_UTXO_COMMITMENT: &[u8] = b"scalar_utxo_v2";

/// Salt derivation. Spec §2.3. 14 bytes.
pub const DOMAIN_SALT: &[u8] = b"scalar_salt_v1";

/// Seed aggregator. Spec §2.3. 15 bytes.
pub const DOMAIN_SEED: &[u8] = b"scalar_seed_v1";

/// NMT peer selection. Spec §2.3. 14 bytes.
pub const DOMAIN_NMT: &[u8] = b"scalar_nmt_v1";
...
pub const DOMAIN_VOTE: &[u8] = b"scalar_vote_v1";

/// Genesis bootstrap. Spec §2.3. 26 bytes.
pub const DOMAIN_GENESIS_BOOTSTRAP: &[u8] = b"scalar_genesis_bootstrap_v1";

/// STARK Fiat-Shamir transcript (transfer). Spec §2.3. 17 bytes.
pub const DOMAIN_STARK_FS: &[u8] = b"scalar_stark_fs_v1";

/// STARK Fiat-Shamir transcript (checkpoint). Spec §2.3. 22 bytes.
pub const DOMAIN_CHECKPOINT_FS: &[u8] = b"scalar_checkpoint_fs_v1";

/// Beacon MAC. Spec §2.3. 16 bytes.
pub const DOMAIN_BEACON: &[u8] = b"scalar_beacon_v1";

/// Seed KDF wallet. Spec §2.3. 9 bytes.
pub const DOMAIN_SEED_KDF: &[u8] = b"scalar_v2";

/// TX ordering. Spec §2.3. 18 bytes.
pub const DOMAIN_TX_ORDER: &[u8] = b"scalar_tx_order_v1";
...
mod tests {
    ...
    fn test_domain_nullifier_len() {
        ...
        assert_eq!(DOMAIN_NULLIFIER.len(), 14);
    }
    ...
    fn test_domain_utxo_commitment_len() {
        ...
        assert_eq!(DOMAIN_UTXO_COMMITMENT.len(), 14);
    }
    ...
    fn test_domain_node_short_len() {
        ...
        assert_eq!(DOMAIN_NODE_SHORT.len(), 20);
    }
    ...
    fn test_domain_anchor_len() {
        ...
        assert_eq!(DOMAIN_ANCHOR.len(), 16);
    }
    ...
    fn test_domain_genesis_bootstrap_len() {
        ...
        assert_eq!(DOMAIN_GENESIS_BOOTSTRAP.len(), 27);
    }
    ...
    fn test_domain_stark_fs_len() {
        ...
        assert_eq!(DOMAIN_STARK_FS.len(), 18);
    }
    ...
    fn test_domain_checkpoint_fs_len() {
        ...
        assert_eq!(DOMAIN_CHECKPOINT_FS.len(), 23);
    }
    ...
    fn test_domain_beacon_len() {
        ...
        assert_eq!(DOMAIN_BEACON.len(), 16);
    }
    ...
    fn test_domain_seed_kdf_len() {
        ...
        assert_eq!(DOMAIN_SEED_KDF.len(), 9);
    }
    ...
    fn test_domain_tx_order_len() {
        ...
        assert_eq!(DOMAIN_TX_ORDER.len(), 18);
    }
